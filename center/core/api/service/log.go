...
func (s *LogManService) HandleLogStream(c *gin.Context) error {
	...
	global.LOG.Info("Log stream task created: id=%s path=%s host=%d", task.ID, path, hostID)
	...
	for {
		...
		case <-heartbeat.C:
			...
			flusher.Flush()
		case <-ctx.Done():
			global.LOG.Info("Log stream closed: id=%s path=%s host=%d", task.ID, path, hostID)
			// 如果是远程读取器，发送停止消息
