<template>
  <a-modal
    ...
    :title="$t('app.store.uninstallLog.title')"
    :footer="false"
    :mask-closable="false"
    :closable="true"
    width="600px"
    ...
    <div class="install-log">
      <div ref="logContentRef" class="log-content">
        ...
        <div v-if="logs.length === 0" class="empty-log">
          {{ $t('app.store.uninstallLog.waitingForLogs') }}
        </div>
      </div>
      <div class="log-footer">
        <a-button type="primary" @click="handleCancel">
          {{ $t('app.store.uninstallLog.close') }}
        </a-button>
      </div>
    </div>
  </a-modal>
...
<script lang="ts" setup>
  ...
  const logFileLogs = (hostId: number, logPath: string) => {
    ...
    eventSource.addEventListener('status', (event: Event) => {
      if (event instanceof MessageEvent) {
        ...
        if (statusValue === 'success') {
          ...
          addLog({
            time: Date.now(),
            message: t('app.store.uninstallLog.progress'),
            level: 'info',
          });
        } else {
          addLog({
    ...
    eventSource.onopen = () => {
      addLog({
        time: Date.now(),
        message: t('app.store.uninstallLog.logConnected'),
        level: 'info',
      });
    };
