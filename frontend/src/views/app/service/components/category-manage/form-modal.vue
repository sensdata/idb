<template>
  <a-modal
    ...
    :title="
      isEdit
        ? $t('app.service.category.action.edit')
  ...
  >
    <a-form ref="formRef" :model="formState" :rules="rules" layout="vertical">
      <a-form-item
        field="name"
        :label="$t('app.service.category.form.name')"
        :rules="[
          {
            required: true,
            message: $t('app.service.category.form.name.required'),
          },
        ]"
      >
        <a-input
          v-model="formState.name"
          :placeholder="$t('app.service.category.form.name.placeholder')"
        />
      </a-form-item>
    </a-form>
  </a-modal>
...
<script setup lang="ts">
  ...
  const rules = ref({
    name: [
      { required: true, message: t('app.service.category.form.name.required') },
    ],
  });
  ...
  const handleOk = async () => {
    try {
      ...
      if (isEdit.value) {
        ...
        Message.success(t('app.service.category.message.update_success'));
      } else {
        await createServiceCategoryApi({
          type: props.type,
          category: formState.name,
        });
        Message.success(t('app.service.category.message.create_success'));
      }
      visible.value = false;
      emit('ok');
    } catch (err: any) {
