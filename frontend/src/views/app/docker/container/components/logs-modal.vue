<template>
  <a-modal
    ...
    :title="$t('app.docker.container.logsModal.title')"
    :footer="false"
    :width="1200"
    destroy-on-close
    @cancel="handleCancel"
...
<script setup lang="ts">
  ...
  function connect(id: string) {
    ...
    const url = resolveApiUrl(`/docker/${hostId}/containers/logs/tail`, {
      id,
    });
    ...
    eventSourceRef.value.addEventListener('error', (event: Event) => {
      if (event.type === 'error') {
        Message.error(t('app.docker.container.logsModal.error'));
      }
    });
  }
