<template>
  <a-drawer
    :visible="visible"
    height="90vh"
    :title="$t('app.crontab.logs.title')"
    placement="bottom"
    @cancel="handleClose"
  >
    <idb-table
    ...
    >
          ...
          <a-spin :loading="expandData[record.id].loading">
            ...
            >
              <a-empty
                ...
                "
                :decrontabion="$t('app.crontab.logs.no_logs')"
              />
              <template v-else>
      ...
      <template #status="{ record }">
        <a-tag :color="record.status === 'success' ? 'green' : 'red'">
          {{ $t(`app.crontab.logs.status.${record.status}`) }}
        </a-tag>
      </template>
    </idb-table>
  </a-drawer>
...
<script setup lang="ts">
  ...
  const columns = [
    ...
    {
      dataIndex: 'cost_time',
      title: t('app.crontab.logs.column.cost_time'),
    },
  ];

  const expandable = reactive({
    title: t('app.crontab.logs.column.logs'),
    width: 80,
  ...
  const loadLogs = async (recordId: number) => {
    ...
    try {
      const res = await getCrontabRunLogApi({
        id: crontabId.value!,
        record_id: recordId,
      });
