...
func (c DockerClient) ImageList() ([]model.Options, error) {
	var (
		list      []image.Summary
...
func (c DockerClient) ImageBuild(req model.ImageBuild, dataDir string, logDir string, logger *log.Log) (string, error) {
	fileName := "Dockerfile"
	if req.From == "edit" {
...
func (c DockerClient) ImagePull(req model.ImagePull, logDir string, logger *log.Log) (string, error) {
	dockerLogDir := path.Join(logDir, "docker_logs")
	if _, err := os.Stat(dockerLogDir); err != nil && os.IsNotExist(err) {
		if err = os.MkdirAll(dockerLogDir, os.ModePerm); err != nil {
			return "", err
...
func (c DockerClient) ImageLoad(req model.ImageLoad) error {
...
func (c DockerClient) ImagePush(req model.ImagePush, logDir string, logger *log.Log) (string, error) {
